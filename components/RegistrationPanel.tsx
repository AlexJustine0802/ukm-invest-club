 ...
 *
 * Rendered by both the public page and the member-area one so a member never
 * gets bounced out to the public site (and its Login / Sign Up header) to
 * answer a form they are already signed in for. `basePath` is the area doing
 * the rendering: it is where the submit comes back to.
 */
export default async function RegistrationPanel({
  slug,
  done,
  basePath,
}: {
  ...
  const full = form.capacity !== null && form._count.responses >= form.capacity;
      ...
      <>
        ...
        {session && (
          <>
            ...
            <Link
              href={
                form.event ? "/account/events?tab=registration" : "/account"
              }
