 ...
 * the top, the write-up runs underneath, and the company keeps its own column
 * to the side.
...
export default async function CareerAlertPage({
  ...
  const alert = await prisma.careerAlert.findFirst({
    ...
    include: {
      // The club's own form, when applications come through us instead of a
      // link. Unpublished means it is not open, so it is not offered.
      applyForm: { select: { slug: true, published: true } },
    },
  });
  if (!alert) notFound();
  ...
  return (
    <>
      ...
      {/* The company sits beside the posting rather than under it, so it is
          still on screen while the description is being read. */}
      <div className="mt-8 grid items-start gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
            ...
            <div className="mt-5 space-y-2">
              ...
              {metaRow(
                ...
                <span className="text-slate-500">
                  {postedLabel(alert.createdAt, now)}
                </span>,
              )}
            </div>

            <div className="mt-6 flex flex-wrap items-center gap-3">
              {applyFormSlug && !closed ? (
                // Our own form: the member never leaves the site, and the
                // answers land under Registrations.
                <Link
                  href={`/account/register/${applyFormSlug}`}
        ...
        {/* Who is hiring. Sticks to the top of the column on a wide screen. */}
